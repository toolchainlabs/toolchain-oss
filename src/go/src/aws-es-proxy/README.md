...
We forked this repo since the [upstream repo](https://github.com/abutaha/aws-es-proxy/) is not well maintained.
This is also helps us dogfood the golang support in Pants.

### Building

TODO: Convert to use pants to package and build docker image.

```shell
docker build -t toolchain-aws-es-proxy:logging .
docker push toolchain-aws-es-proxy:logging
```
...
To access the OpenSearch Dashboard use <https://dev-es.toolchainlabs.com/_dashboards/> (VPN connection is required)
