...
# Based on https://github.com/defnull/multipart/blob/master/multipart.py
# Modified to to fit our use case (work with the request object instead of wsgi)
# Removed memory limits checks (irrenevant for our use case, which is unit tests)
# The repo is not active (last commit about in February 2022)
# It will be easier to tend to this code and debug it this way rather than have it in a 3rd party package.
...
class MultiDict(DictMixin):
    ...
    HTTP headers may repeat with differing values, such as Set-Cookie. We need to remember all values.
    """
...
_special = re.escape('()<>@,;:"\\/[]?={} \t')
_re_special = re.compile(rf"[{_special}]")
_quoted_string = r'"(?:\\.|[^"])*"'  # Quoted string
_value = rf"(?:[^{_special}]+|{_quoted_string})"  # Save or quoted string
...
class MultipartParser:
    ...
    def _lineiter(self):
        ...
        Each line is returned as a (line, line_ending) tuple. If the line does not fit into self.buffer_size,
        line_ending is empty and the rest of the line is returned with the next iteration.
        """
    ...
    def _iterparse(self):
        ...
        for line, nl in lines:
            ...
            if line == separator and not is_tail:
                if part.is_buffered():
                    mem_used += part.size
                else:
                    disk_used += part.size
...
def parse_multipart_request(request) -> tuple[MultiDict, MultiDict]:
    forms, files = MultiDict(), MultiDict()
    if request.method.upper() not in _ALLOWED_METHODS:
        raise MultipartError(f"Request method other than {_ALLOWED_METHODS}: was: {request.method.upper()}")
    ...
    # Work with both httpx and requests (PreparedRequest) objects.
    stream = BytesIO(request.read() if hasattr(request, "read") else request.body)
    if content_type == "multipart/form-data":
        ...
        for part in MultipartParser(stream, boundary, content_length, charset=charset):
        ...
        content = stream.read().decode(charset)
        data = parse_qs(content, keep_blank_values=True, encoding=charset)
