...
from toolchain.github_integration.management.commands.migrate_issues_and_pr_store import check_padding
...
class Command(BaseCommand):
    help = "Delete issues and PRs that are nit stored in s3 to the new padded format."
    ...
    def handle(self, *args, **options) -> None:
        ...
        if repo_fn:
            ...
            deleted_keys_for_repo(repo, dry_run)
        elif options["all"] is True:
            deleted_keys_for_all_repos(dry_run)
        else:
...
def deleted_keys_for_all_repos(dry_run: bool) -> None:
    for repo in Repo.objects.all():
        deleted_keys_for_repo(repo, dry_run)


def deleted_keys_for_repo(repo: Repo, dry_run: bool) -> None:
    ...
    for key in s3.keys_with_prefix(bucket=bucket, key_prefix=pr_base_path):
        ...
        if not s3.exists(bucket=bucket, key=new_key):
            raise ToolchainAssertion(f"No padded key for {key} - checked: {new_key}")
        _logger.info(f"Delete {key} for {repo_fn} {dry_run=}")
        if not dry_run:
            s3.delete_object(bucket=bucket, key=key)
        deleted_keys_count += 1
    if deleted_keys_count:
        _logger.info(f"Deleted {deleted_keys_count} keys for {repo_fn}")
    else:
        _logger.info(f"No keys deleted for {repo_fn}")
