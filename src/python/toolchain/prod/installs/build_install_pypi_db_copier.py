...
class BuildAndDeployPypiDBCopier(ToolchainBinary):
    _PEX_TARGET = "src/python/toolchain/prod/pypi_db_snapshots:pypi-db-snapshots"
    _CHART_PATH = Path("prod/helm/dev-support/pypi-db-snapshot-copier/")
    _ECR_REPO = "pypi-db-snapshots-copier"
    ...
    def __init__(self, cmd_args: Namespace) -> None:
        ...
        self._namespace = "asher"

    def _build_and_push_image(self) -> str:
        builder = PexWrapperBuilder(
            pants_target=self._PEX_TARGET,
            ecr_repo=self._ECR_REPO,
            docker_path="prod/docker/pex-wrapper/Dockerfile.postgres",
        )
    ...
    def run(self) -> int:
        ...
        release = f"{namespace}-pypi-db-snapshot-copier"
...
if __name__ == "__main__":
    BuildAndDeployPypiDBCopier.start()
