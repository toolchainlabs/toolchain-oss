...
from toolchain.prod.installs.deploy_toolchain_spa_dev import BuildAndDeployToolchainSPADev
...
class BuildAndDeployPantsDemositeDev(ToolchainBinary):
    BUCKET = BuildAndDeployToolchainSPADev.BUCKET

    def __init__(self, cmd_args: Namespace) -> None:
        super().__init__(cmd_args)
        self._namespace = cmd_args.namespace or "shared"
        self._action = cmd_args.action
        self._deploy = BuildAndDeployPantsDemoSite.create(
            aws_region=cmd_args.aws_region,
            bucket=self.BUCKET,
            base_key=f"dev/pants-demo-site/{self._namespace}",
...
if __name__ == "__main__":
    BuildAndDeployPantsDemositeDev.start()
