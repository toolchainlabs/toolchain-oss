...
# Based on https://github.com/brouberol/contexttimer/blob/master/contexttimer/__init__.py with some modifications
...
class Timer:
    ...
    def __init__(self, timer=default_timer, factor=1, output=None, fmt="took {:.3f} seconds", prefix=""):
        self.timer = timer
        self.factor = factor
        self.output = output
        self.fmt = fmt
    ...
    def elapsed(self):
        ...
        If the `elapsed` property is called in the context manager scope, the elapsed time between start and property
        access is returned. However, if it is accessed outside of the context manager scope, it returns the elapsed time
        between entering and exiting the scope.

        The `elapsed` property can thus be accessed at different points within the context manager scope, to time
...
def timer(
    ...
    **timer_kwargs,
    ...
    if len(func_or_func_args) == 1 and isinstance(func_or_func_args[0], collections.abc.Callable):
