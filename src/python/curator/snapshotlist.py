...
class SnapshotList(object):
    ...
    def __init__(self, client, repository=None):
        ...
        if not utils.repository_exists(client, repository):
            raise exceptions.FailedExecution("Unable to verify existence of repository " "{0}".format(repository))
    ...
    def __actionable(self, snap):
        self.loggit.debug("Snapshot {0} is actionable and remains in the list.".format(snap))

    def __not_actionable(self, snap):
        self.loggit.debug("Snapshot {0} is not actionable, removing from " "list.".format(snap))
    ...
    def _calculate_ages(self, source="creation_date", timestring=None):
        ...
        if source == "name":
            ...
            raise ValueError("Invalid source: {0}.  " 'Must be "name", or "creation_date".'.format(source))
    ...
    def _sort_by_age(self, snapshot_list, reverse=True):
        ...
        for snap in snapshot_list:
            if self.age_keyfield in self.snapshot_info[snap]:
                ...
                msg = '{0} does not have age key "{1}" in SnapshotList ' " metadata".format(snap, self.age_keyfield)
                self.__excludify(True, True, snap, msg)
    ...
    def most_recent(self):
        ...
        for snapshot in self.snapshots:
            snaptime = utils.fix_epoch(self.snapshot_info[snapshot]["start_time_in_millis"])
    ...
    def filter_by_age(
        ...
        for snapshot in self.working_list():
            if not self.snapshot_info[snapshot][self.age_keyfield]:
                ...
                continue
            msg = 'Snapshot "{0}" age ({1}), direction: "{2}", point of ' "reference, ({3})".format(
                snapshot, utils.fix_epoch(self.snapshot_info[snapshot][self.age_keyfield]), direction, por
            )
            # Because time adds to epoch, smaller numbers are actually older
            # timestamps.
            snapshot_age = utils.fix_epoch(self.snapshot_info[snapshot][self.age_keyfield])
            if direction == "older":
    ...
    def filter_by_count(
        self, count=None, reverse=True, use_age=False, source="creation_date", timestring=None, exclude=True
    ):
        """
    ...
    def filter_period(
        ...
        for snapshot in self.working_list():
            ...
            msg = 'Snapshot "{0}" age ({1}), period start: "{2}", period ' "end, ({3})".format(
                snapshot, age, start, end
            )
            # Because time adds to epoch, smaller numbers are actually older
            # timestamps.
        ...
        for fltr in config["filters"]:
            ...
            self.loggit.debug(
                "Parsed filter args: {0}".format(
                    SchemaCheck(fltr, filters.structure(), "filter", "SnapshotList.iterate_filters").result()
                )
            )
