...
def wait_for_active_shards(action):
    ...
    if action in ["reindex", "shrink"]:
        value = 1
    return {Optional("wait_for_active_shards", default=value): Any(Coerce(int), "all", None)}
...
def wait_interval(action):
    ...
    if action in ["restore", "snapshot", "reindex", "shrink"]:
        value = 9
    return {Optional("wait_interval", default=value): Any(All(Coerce(int), Range(min=minval, max=maxval)), None)}
