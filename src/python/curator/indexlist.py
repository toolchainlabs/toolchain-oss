...
class IndexList(object):
    ...
    def __actionable(self, idx):
        self.loggit.debug("Index {0} is actionable and remains in the list.".format(idx))

    def __not_actionable(self, idx):
        self.loggit.debug("Index {0} is not actionable, removing from list.".format(idx))
    ...
    def _get_index_stats(self):
        ...
        def iterate_over_stats(stats):
            for index in stats["indices"]:
                ...
                self.loggit.debug("Index: {0}  Size: {1}  Docs: {2}".format(index, utils.byte_size(size), docs))
            ...
            if working_list:
                for index in list(working_list.keys()):
                    ...
                    if "creation_date" not in wl["settings"]["index"]:
                        ...
                        s["age"]["creation_date"] = utils.fix_epoch(wl["settings"]["index"]["creation_date"])
                    s["number_of_replicas"] = wl["settings"]["index"]["number_of_replicas"]
    ...
    def _get_field_stats_dates(self, field="@timestamp"):
        ...
        self.filter_empty()
        self.loggit.debug("Getting index date by querying indices for min & max value of " "{0} field".format(field))
        self.empty_list_check()
        index_lists = utils.chunk_index_list(self.indices)
        for l in index_lists:
            for index in l:
                body = {"aggs": {"min": {"min": {"field": field}}, "max": {"max": {"field": field}}}}
                ...
                if response:
                    try:
                        ...
                        raise exceptions.ActionError('Field "{0}" not found in index ' '"{1}"'.format(field, index))
    ...
    def _calculate_ages(self, source=None, timestring=None, field=None, stats_result=None):
        ...
        if source == "name":
            if not timestring:
                raise exceptions.MissingArgument('source "name" requires the "timestring" keyword argument')
            ...
            if not field:
                raise exceptions.MissingArgument('source "field_stats" requires the "field" keyword argument')
            if stats_result not in ["min_value", "max_value"]:
                raise ValueError('Invalid value for "stats_result": {0}'.format(stats_result))
            self.age_keyfield = stats_result
            self._get_field_stats_dates(field=field)
        else:
            raise ValueError(
                "Invalid source: {0}.  " 'Must be one of "name", ' '"creation_date", "field_stats".'.format(source)
            )
    ...
    def _sort_by_age(self, index_list, reverse=True):
        ...
        for index in index_list:
            if self.age_keyfield in self.index_info[index]["age"]:
                ...
                msg = '{0} does not have age key "{1}" in IndexList ' " metadata".format(index, self.age_keyfield)
    ...
    def filter_by_age(
        ...
        for index in self.working_list():
            try:
                ...
                msg = 'Index "{0}" age ({1}), direction: "{2}", point of ' "reference, ({3})".format(
                    index, age, direction, PoR
                )
                ...
                if unit_count_pattern:
                    ...
                    if unit_count_from_index:
                        self.loggit.debug(
                            'Pattern matched, applying unit_count of  "{0}"'.format(unit_count_from_index)
                        )
                ...
                self.loggit.debug('Index "{0}" does not meet provided criteria. ' "Removing from list.".format(index))
    ...
    def filter_forceMerged(self, max_num_segments=None, exclude=True):
        ...
        for index in self.working_list():
            ...
            msg = "{0} has {1} shard(s) + {2} replica(s) " "with a sum total of {3} segments.".format(
    ...
    def filter_empty(self, exclude=True):
        ...
        for index in self.working_list():
            ...
            self.loggit.debug("Index {0} doc count: {1}".format(index, self.index_info[index]["docs"]))
            self.__excludify(condition, exclude, index)
        ...
        for l in index_lists:
            ...
            if working_list:
                for index in list(working_list.keys()):
                    try:
                        has_routing = (
                            working_list[index]["settings"]["index"]["routing"]["allocation"][allocation_type][key]
                            == value
                        )
                    except KeyError:
                        has_routing = False
                    # if has_routing:
                    msg = "{0}: Routing (mis)match: " "index.routing.allocation.{1}.{2}={3}.".format(
    ...
    def filter_by_alias(self, aliases=None, exclude=False):
        ...
        for l in index_lists:
            try:
                ...
                has_alias = list(
                    self.client.indices.get_alias(index=utils.to_csv(l), name=utils.to_csv(aliases)).keys()
                )
            ...
            for index in l:
                if index in has_alias:
                    ...
                    isOrNot = "is not"
                    condition = False
                msg = "{0} {1} associated with aliases: {2}".format(index, isOrNot, aliases)
            ...
            try:
                ...
                if r.groups < 1:
                    ...
                    raise exceptions.ConfigurationError(
                        "More than 1 regular expression group found in {0}".format(pattern)
                    )
                ...
                for index in prune_these:
                    msg = "{0} does not match regular expression {1}.".format(index, pattern)
                    condition = True
                    exclude = True
                    self.__excludify(condition, exclude, index, msg)
    ...
    def filter_by_shards(self, number_of_shards=None, shard_filter_behavior="greater_than", exclude=False):
        ...
        if shard_filter_behavior not in [
            ...
            raise ValueError('Invalid value for "shard_filter_behavior": {0}'.format(shard_filter_behavior))
    ...
    def filter_period(
        ...
        if period_type == "relative":
            ...
            if type(range_from) != type(int()) or type(range_to) != type(int()):
                raise exceptions.ConfigurationError('"range_from" and "range_to" must be integer values')
        else:
        ...
        for index in self.working_list():
            try:
                if source == "field_stats" and intersect:
                    ...
                    age = int(self.index_info[index]["age"][self.age_keyfield])
                    msg = 'Index "{0}" age ({1}), period start: "{2}", period ' 'end, "{3}"'.format(
                        index, age, start, end
                    )
                    # Because time adds to epoch, smaller numbers are actually older
                    # timestamps.
        ...
        for f in filter_dict["filters"]:
            ...
            self.loggit.debug(
                "Parsed filter args: {0}".format(
                    SchemaCheck(f, filters.structure(), "filter", "IndexList.iterate_filters").result()
                )
            )
