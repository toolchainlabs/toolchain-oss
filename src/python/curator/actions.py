    ...
    def add(self, ilo, warn_if_no_indices=False):
        ...
        try:
            ...
            if warn_if_no_indices:
                ...
                self.loggit.warn(
                    "No indices found after processing filters. " "Nothing to add to {0}".format(self.name)
                )
                return
            else:
        ...
        for index in ilo.working_list():
            self.loggit.debug(
                "Adding index {0} to alias {1} with extra settings " "{2}".format(index, self.name, self.extra_settings)
            )
    ...
    def remove(self, ilo, warn_if_no_indices=False):
        ...
        try:
            ...
            if warn_if_no_indices:
                self.warn_if_no_indices = True
                self.loggit.warn(
                    "No indices found after processing filters. " "Nothing to remove from {0}".format(self.name)
                )
                return
            else:
                # Re-raise the exceptions.NoIndices so it will behave as before
        ...
        for index in ilo.working_list():
            if index in aliases:
                ...
                if self.name in aliases[index]["aliases"]:
                    ...
                    self.loggit.debug(
                        "Can not remove: Index {0} is not associated with alias" " {1}".format(index, self.name)
                    )
    ...
    def do_dry_run(self):
        ...
        for item in self.body()["actions"]:
            ...
            self.loggit.info(
                ...
                '"{3}"'.format(job.rstrip("e"), index, "to" if job == "add" else "from", alias)
            )
...
class Allocation(object):
    ...
    def do_action(self):
        ...
        """
        self.loggit.debug(
            "Cannot get change shard routing allocation of closed indices.  " "Omitting any closed indices."
        )
        self.index_list.filter_closed()
        self.index_list.empty_list_check()
        self.loggit.info(
            "Updating {0} selected indices: {1}".format(len(self.index_list.indices), self.index_list.indices)
        )
        self.loggit.info("Updating index setting {0}".format(self.body))
        try:
            index_lists = utils.chunk_index_list(self.index_list.indices)
            for lst in index_lists:
                self.client.indices.put_settings(index=utils.to_csv(lst), body=self.body)
                if self.wfc:
                    self.loggit.debug(
                        "Waiting for shards to complete relocation for indices:" " {0}".format(utils.to_csv(lst))
                    )
                    utils.wait_for_it(
                        self.client, "allocation", wait_interval=self.wait_interval, max_wait=self.max_wait
                    )
        except Exception as err:
            utils.report_failure(err)
...
class Close(object):
    ...
    def do_dry_run(self):
        ...
        Log what the output would be, but take no action.
        """
        utils.show_dry_run(self.index_list, "close", **{"delete_aliases": self.delete_aliases})
            ...
            for lst in index_lists:
                ...
                if self.delete_aliases:
                    ...
                    try:
                        ...
                        self.loggit.warn("Some indices may not have had aliases.  Exception:" " {0}".format(err))
                if not self.skip_flush:
                    try:
                        ...
                        if not self.ignore_sync_failures:
                            ...
                            self.loggit.warn("Ignoring flushed sync failures: " "{0} {1}".format(err.error, err.info))
...
class ClusterRouting(object):
    ...
    def __init__(
        ...
        if routing_type == "allocation":
            if value not in ["all", "primaries", "new_primaries", "none"]:
                raise ValueError('Invalid "value": {0} with "routing_type":' "{1}.".format(value, routing_type))
        elif routing_type == "rebalance":
            if value not in ["all", "primaries", "replicas", "none"]:
                raise ValueError('Invalid "value": {0} with "routing_type":' "{1}.".format(value, routing_type))
        else:
            raise ValueError('Invalid value for "routing_type": {0}.'.format(routing_type))
    ...
    def do_action(self):
        ...
        try:
            ...
            if self.wfc:
                self.loggit.debug("Waiting for shards to complete routing and/or rebalancing")
                utils.wait_for_it(
                    self.client, "cluster_routing", wait_interval=self.wait_interval, max_wait=self.max_wait
                )
        except Exception as err:
            utils.report_failure(err)
...
class CreateIndex(object):
    ...
    def do_dry_run(self):
        ...
        """
        self.loggit.info("DRY-RUN MODE.  No changes will be made.")
        self.loggit.info('DRY-RUN: create_index "%s" with arguments: ' "%s" % (self.name, self.body))
...
class DeleteIndices(object):
    ...
    def __init__(self, ilo, master_timeout=30):
        ...
        if not isinstance(master_timeout, int):
            raise TypeError(
                'Incorrect type for "master_timeout": {0}. ' "Should be integer value.".format(type(master_timeout))
            )
        #: Instance variable.
        #: Internal reference to `ilo`
        self.index_list = ilo
    ...
    def _verify_result(self, result, count):
        ...
        if isinstance(result, list) and result:
            ...
            self.loggit.debug("Successfully deleted all indices on try #{0}".format(count))
            retval = True
        return retval
    ...
    def __chunk_loop(self, chunk_list):
        ...
        for count in range(1, 4):  # Try 3 times
            ...
            self.client.indices.delete(index=utils.to_csv(working_list), master_timeout=self.master_timeout)
            result = [i for i in working_list if i in utils.get_indices(self.client)]
            if self._verify_result(result, count):
                return
            else:
                working_list = result
        self.loggit.error("Unable to delete the following indices after 3 attempts: " "{0}".format(result))
...
class ForceMerge(object):
    ...
    def do_action(self):
        ...
        self.index_list.empty_list_check()
        self.loggit.info(
            "forceMerging {0} selected indices: {1}".format(len(self.index_list.indices), self.index_list.indices)
        )
        try:
            for index_name in self.index_list.indices:
                ...
                if self.delay > 0:
                    self.loggit.info("Pausing for {0} seconds before continuing...".format(self.delay))
...
class IndexSettings(object):
    ...
    def __init__(self, ilo, index_settings={}, ignore_unavailable=False, preserve_existing=False):
        """
        :arg ilo: A :class:`curator.indexlist.IndexList` object
        ...
        if len(self.body) == 1:
            if "index" in self.body:
                if isinstance(self.body["index"], dict):
                    return True
        raise exceptions.ConfigurationError('Bad value for "index_settings": {0}'.format(self.body))
    ...
    def _settings_check(self):
        ...
        for k in self.body["index"]:
            if k in self._static_settings():
                if not self.ignore_unavailable:
                    if open_indices:
                        raise exceptions.ActionError(
                            ...
                            "or setting ignore_unavailable to True".format(k, open_index_list)
                        )
                ...
                self.loggit.warn('"{0}" is not a setting Curator recognizes and may or may ' "not work.".format(k))
...
class Replicas(object):
    ...
    def do_action(self):
        ...
        try:
            index_lists = utils.chunk_index_list(self.index_list.indices)
            for lst in index_lists:
                self.client.indices.put_settings(index=utils.to_csv(lst), body={"number_of_replicas": self.count})
                if self.wfc and self.count > 0:
                    self.loggit.debug(
                        "Waiting for shards to complete replication for " "indices: {0}".format(utils.to_csv(lst))
                    )
                    utils.wait_for_it(self.client, "replicas", wait_interval=self.wait_interval, max_wait=self.max_wait)
        except Exception as err:
            utils.report_failure(err)
...
class Rollover(object):
    ...
    def __init__(self, client, name, conditions, new_index=None, extra_settings=None, wait_for_active_shards=1):
        """
        :arg client: An :class:`opensearchpy.OpenSearch` client object
        ...
        if not isinstance(extra_settings, dict) and extra_settings is not None:
            raise exceptions.ConfigurationError('"extra_settings" must be a dictionary or None')
        utils.verify_client_object(client)
        #: Instance variable.
        ...
        if utils.rollable_alias(client, name):
            ...
            raise ValueError(
                "Unable to perform index rollover with alias " '"{0}". See previous logs for more details.'.format(name)
            )
    ...
    def log_result(self, result):
        ...
        if result["dry_run"] and success:  # log "successful" dry-run
            ...
            self.loggit.info(
                "{0}Rollover conditions not met. Index {1} not rolled over.".format(dryrun_string, result["old_index"])
            )
...
class DeleteSnapshots(object):
    ...
    def do_dry_run(self):
        ...
        for snap in self.snapshot_list.snapshots:
            self.loggit.info("DRY-RUN: delete_snapshot: {0} with arguments: {1}".format(snap, mykwargs))
    ...
    def do_action(self):
        ...
        if not utils.safe_to_snap(
            self.client, repository=self.repository, retry_interval=self.retry_interval, retry_count=self.retry_count
        ):
...
class Reindex(object):
    ...
    def __init__(
        ...
        if self.migration:
            if not self.remote and not self.mpfx and not self.msfx:
                raise exceptions.ConfigurationError(
                    "MIGRATION can only be used locally with one or both of " "migration_prefix or migration_suffix."
                )
        ...
        # Remote section
            ...
            for k in ["host", "username", "password"]:
                rclient_info[k] = self.body["source"]["remote"][k] if k in self.body["source"]["remote"] else None
            ...
            try:
                ...
                raise exceptions.ConfigurationError(
                    "Host must be in the form [scheme]://[host]:[port] but " "was [{0}]".format(rhost)
                )
            rhttp_auth = (
            ...
            if rhost[:5] == "http:":
                ...
                raise exceptions.ConfigurationError(
                    "Host must be in URL format. You provided: " "{0}".format(rclient_info["host"])
                )
        ...
        self.loggit.debug("Reindexing indices: {0}".format(self.body["source"]["index"]))
    ...
    def get_processed_items(self, task_id):
        ...
        try:
            ...
            raise exceptions.CuratorException(
                'Unable to obtain task information for task_id "{0}". Exception ' "{1}".format(task_id, err)
            )
    ...
    def _post_run_quick_check(self, index_name, task_id):
        ...
        if processed_items == 0:
            ...
            if not index_exists and not alias_instead:
                ...
                raise exceptions.FailedExecution(
                    'Reindex failed. The index or alias identified by "{0}" was ' "not found.".format(index_name)
                )
    ...
    def do_action(self):
        ...
        try:
            ...
            raise exceptions.NoIndices("Source index must be list of actual indices. " "It must not be an empty list.")
        except Exception as err:
            utils.report_failure(err)
...
class Snapshot(object):
    ...
    def __init__(
        ...
        if not utils.repository_exists(ilo.client, repository=repository):
            raise exceptions.ActionError("Cannot snapshot indices to missing repository: " "{0}".format(repository))
        if not name:
            raise exceptions.MissingArgument('No value for "name" provided.')
        #: Instance variable.
        #: The Elasticsearch Client object derived from `ilo`
    ...
    def get_state(self):
        ...
        """
        try:
            self.state = self.client.snapshot.get(repository=self.repository, snapshot=self.name)["snapshots"][0][
                "state"
            ]
            return self.state
        except IndexError:
            raise exceptions.CuratorException(
                'Snapshot "{0}" not found in repository ' '"{1}"'.format(self.name, self.repository)
            )
    ...
    def do_action(self):
        ...
        try:
            self.loggit.info('Creating snapshot "{0}" from indices: {1}'.format(self.name, self.index_list.indices))
            # Always set wait_for_completion to False. Let 'utils.wait_for_it' do its
            # thing if wait_for_completion is set to True. Report the task_id
            # either way.
            self.client.snapshot.create(
                repository=self.repository, snapshot=self.name, body=self.body, wait_for_completion=False
            )
...
class Restore(object):
    ...
    def do_dry_run(self):
        ...
        self.loggit.info(
            ...
            "{2}".format(self.repository, self.name, {"wait_for_completion": self.wfc, "body": self.body})
        )

        for index in self.indices:
            if self.rename_pattern and self.rename_replacement:
                replacement_msg = "as {0}".format(re.sub(self.rename_pattern, self.py_rename_replacement, index))
            else:
                replacement_msg = ""
    ...
    def do_action(self):
        ...
        try:
            self.loggit.info('Restoring indices "{0}" from snapshot: {1}'.format(self.indices, self.name))
            # Always set wait_for_completion to False. Let 'utils.wait_for_it' do its
            # thing if wait_for_completion is set to True. Report the task_id
            # either way.
...
class Shrink(object):
    ...
    def __init__(
        ...
        if utils.get_version(self.client) >= (6, 1, 0):
            self._merge_extra_settings(
                {"settings": {"index.routing.allocation.require._name": None, "index.blocks.write": None}}
            )

    def _merge_extra_settings(self, extra_settings):
        self.loggit.debug("Adding extra_settings to shrink body: " "{0}".format(extra_settings))
        ...
        if extra_settings:
            try:  # Apply any remaining keys, should there be any.
                ...
                raise exceptions.ConfigurationError(
                    'Unable to apply extra settings "{0}" to shrink body. Exception: {1}'.format(extra_settings, err)
                )
    ...
    def route_index(self, idx, allocation_type, key, value):
        ...
        try:
            ...
            if self.wait_for_rebalance:
                ...
                utils.wait_for_it(
                    self.client, "relocate", index=idx, wait_interval=self.wait_interval, max_wait=self.max_wait
                )
        except Exception as err:
            utils.report_failure(err)
    ...
    def _check_space(self, idx, dry_run=False):
        ...
        if padded < self.shrink_node_avail:
            ...
            error_msg = (
                ...
                "exceed space available. Required: {1}, available: {2}".format(idx, padded, self.shrink_node_avail)
            )
    ...
    def _check_shard_factor(self, idx, src_shards, dry_run=False):
        ...
        if not self.number_of_shards in factors:
            error_msg = '"{0}" is not a valid factor of {1} shards.  Valid values are ' "{2}".format(
                self.number_of_shards, src_shards, factors
            )
            self.__log_action(error_msg, dry_run)
            ...
            for lst in index_lists:
                for idx in lst:  # Shrink can only be done one at a time...
                    ...
                    self.loggit.info(
                        ...
                        "wait_for_active_shards={3}".format(idx, target, self.body, self.wait_for_active_shards)
                    )
                    ...
                    if self.copy_aliases:
                        self.loggit.info(
                            'DRY-RUN: Copy source index aliases "{0}"'.format(self.client.indices.get_alias(idx))
                        )
    ...
    def do_action(self):
        ...
        self.index_list.empty_list_check()
        self.loggit.info(
            "Shrinking {0} selected indices: {1}".format(len(self.index_list.indices), self.index_list.indices)
        )
        try:
            index_lists = utils.chunk_index_list(self.index_list.indices)
            for lst in index_lists:
                for idx in lst:  # Shrink can only be done one at a time...
                    ...
                    try:
                        self.client.indices.shrink(
                            index=idx, target=target, body=self.body, wait_for_active_shards=self.wait_for_active_shards
                        )
                        ...
                        if self.wfc:
                            ...
                            if self.wait_for_rebalance:
                                utils.wait_for_it(
                                    self.client, "shrink", wait_interval=self.wait_interval, max_wait=self.max_wait
                                )
                            else:
                                utils.wait_for_it(
                                    self.client,
                        ...
                        if self.client.indices.exists(index=target):
                            self.loggit.error(
                                'Deleting target index "{0}" due to failure to complete ' "shrink".format(target)
                            )
                            self.client.indices.delete(index=target)
                        raise exceptions.ActionError('Unable to shrink index "{0}" -- Error: {1}'.format(idx, err))
