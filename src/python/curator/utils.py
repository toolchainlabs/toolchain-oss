...
def verify_client_object(test):
    ...
    if str(type(test)) == "<class 'mock.Mock'>" or str(type(test)) == "<class 'mock.mock.Mock'>":
        pass
...
def fix_epoch(epoch):
    ...
    try:
        ...
        raise ValueError("Invalid epoch received, unable to convert {0} to int. {1}".format(epoch, err))
...
def date_range(unit, range_from, range_to, epoch=None, week_starts_on="sunday"):
    ...
    if unit not in acceptable_units:
        raise exceptions.ConfigurationError('"unit" must be one of: {0}'.format(acceptable_units))
    if not range_to >= range_from:
        raise exceptions.ConfigurationError('"range_to" must be greater than or equal to "range_from"')
    if not epoch:
        epoch = time.time()
    epoch = fix_epoch(epoch)
    ...
    if unit == "hours":
        point_of_ref = datetime(
            raw_point_of_ref.year, raw_point_of_ref.month, raw_point_of_ref.day, raw_point_of_ref.hour, 0, 0
        )
        start_delta = timedelta(hours=origin)
    if unit == "days":
        point_of_ref = datetime(raw_point_of_ref.year, raw_point_of_ref.month, raw_point_of_ref.day, 0, 0, 0)
    ...
    LOGGER.debug("End ISO8601 = {0}".format(datetime.utcfromtimestamp(end_epoch).isoformat()))
    return (start_epoch, end_epoch)


def absolute_date_range(unit, date_from, date_to, date_from_format=None, date_to_format=None):
    """
    Get the epoch start time and end time of a range of ``unit``s, reckoning the
    start of the week (if that's the selected unit) based on ``week_starts_on``,
    which can be either ``sunday`` or ``monday``.
    ...
    try:
        ...
        raise exceptions.ConfigurationError(
            'Unable to parse "date_to" {0} and "date_to_format" {1}. ' "Error: {2}".format(date_to, date_to_format, err)
        )
    # We have to iterate to one more month, and then subtract a second to get
    # the last day of the correct month
    if unit == "months":
    ...
    # It's not months or years, which have inconsistent reckoning...
        ...
        end_epoch = get_point_of_reference(unit, -1, epoch=datetime_to_epoch(end_date)) - 1
...
def check_version(client):
    ...
    version_number = get_version(client)
    LOGGER.debug("Detected Elasticsearch version {0}".format(".".join(map(str, version_number))))
    if version_number >= settings.version_max() or version_number < settings.version_min():
        LOGGER.error(
...
def process_host_args(data):
    ...
    if "host" in data and "hosts" in data:
        raise exceptions.ConfigurationError('Both "host" and "hosts" are defined.  Pick only one.')
...
def process_master_only_arg(data):
    ...
    if master_only:
        if len(data["hosts"]) > 1:
            LOGGER.error(
                '"master_only" cannot be true if more than one host is ' "specified. Hosts = {0}".format(data["hosts"])
            )
            raise exceptions.ConfigurationError(
                '"master_only" cannot be true if more than one host is ' "specified. Hosts = {0}".format(data["hosts"])
            )
...
def process_auth_args(data):
    ...
    if http_auth:
        ...
        LOGGER.warn('Use of "http_auth" is deprecated. Please use "username" and "password" instead.')
...
def process_ssl_args(data):
    ...
    data["ssl_no_validate"] = False if "ssl_no_validate" not in data else data["ssl_no_validate"]
...
def process_aws_args(data):
    ...
    data["aws_sign_request"] = False if "aws_sign_request" not in data else data["aws_sign_request"]
    ...
    if has_requests_module:
        if data["aws_key"]:
            ...
            data["http_auth"] = AWS4Auth(
                data["aws_key"], data["aws_secret_key"], data["aws_region"], "es", session_token=data["aws_token"]
            )
    return data
...
def do_version_check(client, skip):
    ...
    """
    if skip:
        LOGGER.warn("Skipping Elasticsearch version verification. This is " "acceptable for remote reindex operations.")
    else:
...
def show_dry_run(ilo, action, **kwargs):
    ...
    LOGGER.info('(CLOSED) indices may be shown that may not be acted on by action "{0}".'.format(action))
...
def get_snapshot_data(client, repository=None):
    ...
    try:
        ...
        raise exceptions.FailedExecution(
            "Unable to get snapshot information from repository: " "{0}. Error: {1}".format(repository, err)
        )
...
def snapshot_in_progress(client, repository=None, snapshot=None):
    ...
    inprogress = [snap["snapshot"] for snap in allsnaps if "state" in snap.keys() and snap["state"] == "IN_PROGRESS"]
    if snapshot:
        ...
        if not inprogress:
            ...
            raise exceptions.CuratorException("More than 1 snapshot in progress: {0}".format(inprogress))
    return retval
...
def create_snapshot_body(indices, ignore_unavailable=False, include_global_state=True, partial=False):
    """
...
def create_repo_body(
    ...
    **kwargs,
    ...
    maybes = ["compress", "chunk_size", "max_restore_bytes_per_sec", "max_snapshot_bytes_per_sec"]
...
def create_repository(client, **kwargs):
    ...
    try:
        ...
        if not result:
            ...
            raise exceptions.FailedExecution(
                "Unable to create repository {0}.  " "A repository with that name already exists.".format(repository)
            )
    except opensearchpy.TransportError as err:
        raise exceptions.FailedExecution(
            """
    ...
    try:
        ...
        try:
            if err.status_code == 404:
                ...
                msg = "--- Got a {0} response from Elasticsearch.  " "Error message: {1}".format(
                    err.status_code, err.error
                )
        except AttributeError:
            msg = "--- Error message: {0}".format(err)
        raise exceptions.ActionError("Failed to verify all nodes have repository access: {0}".format(msg))
...
def validate_filters(action, filters):
    ...
    for fil in filters:
        if fil["filtertype"] not in filtertypes:
            raise exceptions.ConfigurationError(
                '"{0}" filtertype is not compatible with action "{1}"'.format(fil["filtertype"], action)
            )
...
def validate_actions(data):
    ...
    for action_id in root["actions"]:
        ...
        clean_options = SchemaCheck(
            prune_nones(valid_structure["options"]), options.get_schema(current_action), "options", loc
        ).result()
...
def health_check(client, **kwargs):
    ...
    for k in klist:
        ...
        if not hc_data[k] == kwargs[k]:
            LOGGER.debug('NO MATCH: Value for key "{0}", health check data: ' "{1}".format(kwargs[k], hc_data[k]))
            response = False
        else:
            LOGGER.debug('MATCH: Value for key "{0}", health check data: ' "{1}".format(kwargs[k], hc_data[k]))
    if response:
...
def snapshot_check(client, snapshot=None, repository=None):
    ...
    """
    try:
        state = client.snapshot.get(repository=repository, snapshot=snapshot)["snapshots"][0]["state"]
    except Exception as err:
        raise exceptions.CuratorException(
...
def restore_check(client, index_list):
    ...
    for chunk in chunk_index_list(index_list):
        try:
            ...
            raise exceptions.CuratorException(
                "Unable to obtain recovery information for specified indices. " "Error: {0}".format(err)
            )
    ...
    for index in response:
        for shard in range(0, len(response[index]["shards"])):
            ...
            if response[index]["shards"][shard]["stage"] != "DONE":
                LOGGER.info(
                    'Index "{0}" is still in stage "{1}"'.format(index, response[index]["shards"][shard]["stage"])
                )
                return False
...
def task_check(client, task_id=None):
    ...
    if task["action"] == "indices:data/write/reindex":
        ...
        if "response" in task_data:
            ...
            if response["failures"]:
                raise exceptions.FailedReindex("Failures found in reindex response: {0}".format(response["failures"]))
    ...
    if completed:
        ...
        time_string = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.localtime(completion_time / 1000))
        ...
        LOGGER.info(
            'Task "{0}" with task_id "{1}" has been running for ' "{2} seconds".format(descr, task_id, running_time)
        )
        return False
...
def wait_for_it(
    ...
    if action not in wait_actions:
        raise exceptions.ConfigurationError('"action" must be one of {0}'.format(wait_actions))
    if action == "reindex" and task_id is None:
        raise exceptions.MissingArgument('A task_id must accompany "action" {0}'.format(action))
    ...
    if action == "restore" and index_list is None:
        raise exceptions.MissingArgument('An index_list must accompany "action" {0}'.format(action))
    elif action == "reindex":
        try:
            ...
            raise exceptions.CuratorException("Unable to find task_id {0}. Exception: {1}".format(task_id, err))
    ...
    while True:
        ...
        # Success
        if response:
            LOGGER.debug('Action "{0}" finished executing (may or may not have been ' "successful)".format(action))
            result = True
            break
        # Not success, and reached maximum wait (if defined)
        elif (max_wait != -1) and (elapsed >= max_wait):
            LOGGER.error('Unable to complete action "{0}" within max_wait ({1}) ' "seconds.".format(action, max_wait))
            break
    ...
    if not result:
        raise exceptions.ActionTimeout(
            'Action "{0}" failed to complete in the max_wait period of ' "{1} seconds".format(action, max_wait)
        )
    ...
    if len(list(response.keys())) > 1:
        for index in list(response.keys()):
            try:
                ...
                raise exceptions.FailedExecution("Invalid alias: is_write_index not found in 1 to many alias")
    else:
