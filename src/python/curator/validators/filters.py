...
def filtertype():
    return {
        Required("filtertype"): Any(
            In(settings.all_filtertypes()), msg="filtertype must be one of {0}".format(settings.all_filtertypes())
        )
    }
    ...
    def f(v):
        ...
        for idx in range(0, len(v)):
            ...
            filter_dict = SchemaCheck(
                pruned, single(action, pruned), "filter", "{0}, filter #{1}: {2}".format(location, idx, pruned)
            ).result()
