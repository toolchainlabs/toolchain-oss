...
/// Retry an operation using exponential back-off to delay between attempts.
#[inline]
...
mod tests {
    ...
    async fn retry_call_works_as_expected() {
        ...
        assert_eq!(client.values.lock().unwrap().len(), 1);
        ...
        assert_eq!(client.values.lock().unwrap().len(), 2);
